...
Можно создать коллекцию элементов и встроить её в JSX с помощью фигурных скобок {}.
...
```
import { Link } from "react-router-dom"; \\ компонент - упрощенна версия NavLink без элементов стилизации при переходе на ссылку
...
1) Reducer
JS функция, которая преобразует state. Всегда возвращает новый state

```
// обычно создается начальное состояние, которое по умолчанию будет равно state
const initialState = 
  {
    counter: 0
  }

// функция reducer - 2 параметра: 1) state (по умолчанию обычно равняется начальному состоянию) и 2) action - объект с единственным свойством (параметром) - type 
const reducer = (state = initialState, action) => 
  {
  if (action.type === 'ADD')        // проверки типа action
    {
...
3) Store
Redux поддерживает идеологию, что состояние для всего приложения хранится в едином месте в виде JS объекта
```
const store = redux.creasteStore(reducer)

store.subscribe (() =>
{
  console.log('Subscribe', store.getState()) // метод, который позволяет `подписываьтся` на store при его изменении  
})

```
store.getState() - метод - возвращает состояние 

4) Actions 
Объект, через который происходит изменение состояния
```
const addCounter = 
  {
    type: 'ADD'
